...
// HTTP 429 from bare curl — Playwright with stealth bypasses.
// Shares Expedia property IDs (same platform).
// ============================================================
...
export class HotelsDotComScraper extends BaseScraper {
  ...
  async scrapeHotel(hotelName: string, checkIn: Date, checkOut: Date): Promise<ScrapedRate[]> {
    ...
    try {
      ...
      for (const sel of ['[data-stid="modal-close"]', '[aria-label="Close"]']) {
        try {
      ...
      for (const room of allRooms) {
        try {
          ...
          const inclEl = await room.$('[class*="inclusion"], [class*="amenity"], [class*="benefit"]');
          const mealText = (await inclEl?.textContent())?.toLowerCase() || '';
